...
export class TagService extends BaseService {
   ...
   */
  async getAllTags(
    includeUsageStats: boolean = false,
    ...
    return this.executeWithLogging(
      async () => {
        if (includeUsageStats) {
          ...
          const tags = await this.withRetry(
            () => tagApi.getAll(),
            "getAllTags",
          );
   ...
   */
  async createTag(
    tagData: Omit<TagCreate, "user_id">,
    ...
    return this.executeWithLogging(
      async () => {
        try {
          ...
          const finalColor =
            validatedColor || this.getNextAvailableColor(existingTags);
      ...
      async () => {
        try {
          ...
          if (newsletters.length > 0) {
            logger.warn(
              `Deleting tag with ${newsletters.length} associated newsletters`,
              {
  ...
  private getNextAvailableColor(existingTags: Tag[]): string {
    const usedColors = new Set(
      existingTags.map((tag) => tag.color.toLowerCase()),
    );
  ...
  private async getNewslettersWithTag(
    tagId: string,
