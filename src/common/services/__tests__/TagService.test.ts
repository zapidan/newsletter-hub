...
describe("TagService", () => {
  ...
  describe("getTag", () => {
    ...
    it("should throw NotFoundError when tag not found", async () => {
      ...
      await expect(service.getTag("tag-1")).rejects.toThrow(
        "Tag with ID tag-1 not found",
      );
    });

    it("should validate tag ID", async () => {
      await expect(service.getTag("")).rejects.toThrow(
  ...
  describe("getOrCreateTag", () => {
    it("should get existing tag", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.tag).toEqual(mockTag);
      expect(mockTagApi.getOrCreate).toHaveBeenCalledWith(
        "Test Tag",
        undefined,
      );
    });
  ...
  describe("updateNewsletterTags", () => {
    it("should update newsletter tags successfully", async () => {
      ...
      const result = await service.updateNewsletterTags("newsletter-1", [
        "tag-1",
      ]);
    ...
    it("should enforce maximum tag limit", async () => {
      ...
      const result = await service.updateNewsletterTags(
        "newsletter-1",
        manyTagIds,
      );
    ...
    it("should validate inputs", async () => {
      const result = await service.updateNewsletterTags("", ["tag-1"]);
      expect(result.success).toBe(false);
      expect(result.error).toBe("Tag with ID tag-1 not found");

      await expect(
        service.updateNewsletterTags(
          "newsletter-1",
