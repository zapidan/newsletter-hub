...
export interface UseLoadingStatesOptions {
  ...
  onStateChange?: (key: string, state: LoadingState, info: LoadingStateInfo) => void;
}
...
export const useLoadingStates = (options: UseLoadingStatesOptions = {}): LoadingStatesReturn => {
  const {
    enableDebug = false,
    trackTiming = true,
    maxHistorySize = 100,
  ...
  const historyRef = useRef<Array<{ key: string; state: LoadingState; timestamp: Date; info: LoadingStateInfo }>>([]);
  ...
  // Update state helper
  const updateState = useCallback((key: string, newState: LoadingState, error?: Error) => {
    setStates(prev => {
      ...
      if (trackTiming && newInfo.startTime && newInfo.endTime) {
        newInfo.duration = newInfo.endTime.getTime() - newInfo.startTime.getTime();
      }
      ...
      // Debug logging
      if (enableDebug) {
        console.log(`🔄 Loading state changed: ${key} -> ${newState}`, {
  ...
  const isIdle = useCallback((key: string): boolean => {
  ...
  const getInfo = useCallback((key: string): LoadingStateInfo => {
    return states[key] || { state: 'idle' };
  }, [states]);
  ...
  // Bulk operations
  const setBulkLoading = useCallback((keys: string[]) => {
    keys.forEach(key => setLoading(key));
  }, [setLoading]);

  const setBulkSuccess = useCallback((keys: string[]) => {
    keys.forEach(key => setSuccess(key));
  }, [setSuccess]);

  const setBulkError = useCallback((keys: string[], error?: Error) => {
    keys.forEach(key => setError(key, error));
  }, [setError]);

  const setBulkIdle = useCallback((keys: string[]) => {
  ...
  const getStats = useCallback(() => {
    ...
    const activeLoading = Object.values(states).filter(info => info.state === 'loading').length;

    const completedOperations = history.filter(entry =>
      entry.info.duration !== undefined
    );

    const averageDuration = completedOperations.length > 0
      ? completedOperations.reduce((sum, entry) => sum + (entry.info.duration || 0), 0) / completedOperations.length
      : 0;

    const errorOperations = history.filter(entry => entry.state === 'error').length;
...
export const useNewsletterLoadingStates = () => {
  ...
  const isNewsletterLoading = useCallback((action: string, newsletterId: string) => {
    return loadingStates.isLoading(`${action}-${newsletterId}`);
  }, [loadingStates]);

  const isAnyNewsletterLoading = useCallback((newsletterId: string) => {
