...
export const useErrorHandling = (options: UseErrorHandlingOptions = {}) => {
  ...
  const handleError = useCallback((
    ...
    setErrorState(prev => ({
      ...
      errorHistory: [appError, ...prev.errorHistory].slice(0, maxErrorHistory),
    }));
  ...
  const getErrorStats = useCallback(() => {
    const categoryCounts = errorState.errorHistory.reduce((acc, error) => {
      const category = error.category || 'unknown';
      acc[category] = (acc[category] || 0) + 1;
      return acc;
    }, {} as Record<ErrorCategory, number>);

    const severityCounts = errorState.errorHistory.reduce((acc, error) => {
