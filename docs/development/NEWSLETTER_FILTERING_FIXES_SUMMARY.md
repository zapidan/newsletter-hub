...
- **DELETED**: `getTotalCountBySource()` was removed as it was a duplicate of `countBySource()` which already had the correct archived filtering
- **NEVER IMPLEMENTED**: Hook was planned but never created
- **NEVER IMPLEMENTED**: Query key was planned but never added
- **NEVER IMPLEMENTED**: UI integration was planned but never completed
...
- ~~**Total Count Integration**: Added `useTotalCountsBySource` hook usage~~ **NEVER IMPLEMENTED**: Planned but never completed
...
- **DELETED**: Removed `getTotalCountBySource()` as it was a duplicate of existing `countBySource()` method
- **Enhanced Logging**: Added debug logging for count calculations

#### 4. `src/common/hooks/useUnreadCount.ts`

- ~~**New Hook**: Added `useTotalCountsBySource()` hook~~ **NEVER IMPLEMENTED**: Planned but never created
...
- ~~**New Query Key**: Added `totalCountsBySource` query key~~ **NEVER IMPLEMENTED**: Planned but never added
